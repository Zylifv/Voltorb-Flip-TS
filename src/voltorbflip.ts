...
let currentCoins = document.getElementById(
  "currentCoins"
...
const notesBoxOuter = document.getElementById(
  "notes-box-outer"
) as HTMLDivElement;
...
function startGame() {
  ...
  for (let i: number = 0; i < boxes.length; i++) {
    ...
    let boxArr = [];
    boxArr.push(boxes);
    boxArr.forEach(function (nodelist) {
      //assigning id's to each box instead of having to manually type and organise each time
  ...
  boxes.forEach((box: any) =>
    box.append(
      scoreOneNoteImg.cloneNode(true),
      scoreTwoNoteImg.cloneNode(true),
      scoreThreeNoteImg.cloneNode(true),
      scoreBombNoteImg.cloneNode(true)
    )
  );
  ...
  const greenColumnInfo = document.getElementById(
    "maxScore7"
  ) as HTMLDivElement;
  const yellowColumnInfo = document.getElementById(
    "maxScore8"
  ) as HTMLDivElement;
  const blueColumnInfo = document.getElementById("maxScore9") as HTMLDivElement;
  const purpleColumnInfo = document.getElementById(
    "maxScore10"
  ) as HTMLDivElement;
  ...
  const yellowRowVoltorb = document.getElementById(
    "voltorb3"
  ) as HTMLDivElement;
  const blueRowVoltorb = document.getElementById("voltorb4") as HTMLDivElement;
  const purpleRowVoltorb = document.getElementById(
    "voltorb5"
  ) as HTMLDivElement;
  //Voltorb column number info
  const redColumnVoltorb = document.getElementById(
    "voltorb6"
  ) as HTMLDivElement;
  const greenColumnVoltorb = document.getElementById(
    "voltorb7"
  ) as HTMLDivElement;
  const yellowColumnVoltorb = document.getElementById(
    "voltorb8"
  ) as HTMLDivElement;
  const blueColumnVoltorb = document.getElementById(
    "voltorb9"
  ) as HTMLDivElement;
  const purpleColumnVoltorb = document.getElementById(
    "voltorb10"
  ) as HTMLDivElement;
  ...
  for (let i: number = 0; i < boxes.length; i++) {
    ...
    currentBoxesWithoutBombs = currentLevelCoins.filter(
      (el) => el !== 0
    ).length;
...
for (let i: number = 0; i < boxes.length; i++) {
  ...
  boxes[i].addEventListener("click", () => {
    ...
    if (boxes[i].value == 0) {
      ...
      elem.src =
        "https://img.itch.zone/aW1nLzEyOTA5NTUxLnBuZw==/315x250%23c/yslQia.png"; //adds a nice picture of voltorb to show a boom
    ...
    if (currentLevelPoints >= currentBoxesWithoutBombs) {
      ...
      totalCoins.innerText = `${
        Number(totalCoins.innerText) + currentLevelCoins
      }`;
      boxes.forEach((el: any) => (el.disabled = true));
      ...
      boxes.forEach((box: any) => {
        if (box.value == 0) {
          ...
          elem.src =
            "https://img.itch.zone/aW1nLzEyOTA5NTUxLnBuZw==/315x250%23c/yslQia.png";
        }
      });
    }
  });
...
notesBoxOuter.addEventListener("click", () => {
  alert(
    "Click the symbol you wish to mark on a desired box to keep track of possible points or Voltorbs!"
  );
});
